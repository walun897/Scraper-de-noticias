...
def stratified_balance(df: pd.DataFrame) -> pd.DataFrame:
    df=df.copy(); df=df[df["label"].isin(["true","false","doubtful"])].reset_index(drop=True)
    if df.empty: return df
    n=len(df); targets={k:int(math.floor(n*r)) for k,r in TARGET_RATIOS.items()}
    slices={}
    for lab,t in targets.items():
        pool=df[df["label"]==lab]
        slices[lab]= pool if len(pool)<=t else pool.sample(n=t,random_state=42)
    allocated=sum(len(s) for s in slices.values()); deficit=n-allocated
    if deficit>0:
