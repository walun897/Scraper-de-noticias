...
def fetch_html(url: str) -> BeautifulSoup | None:
    try:
        resp = requests.get(url, timeout=TIMEOUT_SECS, headers={
            "User-Agent": "Mozilla/5.0 (compatible; NewsScraper/1.1)"
        })
...
def scrape_html_cards(url: str, selectors: Dict[str, str], limit_days: int = DEFAULT_LOOKBACK_DAYS) -> List[Dict]:
    ...
    for card in soup.select(selectors["item"]):
        def sel(css):
            ...
            n = card.select_one(css)
            return n.get_text(strip=True) if n else None
        def href(css):
            if not css: return None
            n = card.select_one(css)
            return n.get("href") if n else None
    ...
    for item in soup.select("item"):
        ...
        for f in label_field_candidates:
            ...
            if n and n.get_text(strip=True):
                label_raw = n.get_text(strip=True)
                break
